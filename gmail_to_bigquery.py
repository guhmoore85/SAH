...
GMAIL_USER = os.getenv("GMAIL_USER", "hmoore@stopaapihate.org")
...
# BigQuery scopes needed
BQ_SCOPES = ["https://www.googleapis.com/auth/bigquery"]

# Gmail scopes – domain-wide delegation lets a service account impersonate
# a user on the workspace domain.
...
def get_gmail_service():
    """Build an authorized Gmail API service using domain-wide delegation."""
...
def run(mode: str) -> bool:
    ...
    """
    log.info("=== Gmail → BigQuery pipeline  |  mode=%s ===", mode)
